...
(function(){
	if($.Datatype){
		$.extend($.Tipmsg.w,{
			...
			"notvalued":"不能含有特定值！",
			"idcard":"身份证号码不对！",
			"adminPassword":"长度8-32且必须包含大写字母、小写字母、特殊字符以及数字！不能包含账号信息和空格！",
			"adminChangePassword":"长度8-32且必须包含大写字母、小写字母、特殊字符以及数字！不能包含空格！"
		});
		...
		$.extend($.Datatype,{
			...
			*/
			...
			/*
			  ...
			  datatype为内置的一些正则表达式的引用。
			*/
			"adminPassword":function(gets,obj,curform,datatype){
					/*
					当前文本框的值不能含有指定文本框的值，如注册时设置的密码里不能包含用户名
					注意需要给表单元素绑定with属性来指定要比较的表单元素，可以是clas，id或者是name属性值
					<input type="text" name="username" id="name" class="name" />
				...
				*/
				...
				var scores =0;
				scores = ratepasswd(gets);
				if(scores<0){
					return false;
				}else if(scores < 35 && scores >=0){
					return false;
			...
			/*
			  ...
			  datatype为内置的一些正则表达式的引用。
			*/
			"adminChangePassword":function(gets,obj,curform,datatype){
				//判断空格
				/*if (gets.indexOf(" ") !=-1) {
					return false;
				}*/
