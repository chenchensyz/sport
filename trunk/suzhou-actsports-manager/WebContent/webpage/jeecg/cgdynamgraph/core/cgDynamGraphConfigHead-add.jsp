		...
		<tr>
			...
			<td class="value" colspan="5"><textarea rows="5" cols="150" id="cgrSql" name="cgrSql" datatype="*"></textarea> <span class="Validform_checktip"></span>
						<p>&nbsp;&nbsp;&nbsp;&nbsp;您可以键入“${abc}”作为一个参数，这里abc是参数的名称。例如：<br/>
							&nbsp;&nbsp;&nbsp;&nbsp;select broswer name,count(1) value, DATE_FORMAT(operatetime, '%Y-%m-%d') date from t_s_log  where id = <%="${abc}"%> group by broswer。<br/>
							&nbsp;&nbsp;&nbsp;&nbsp;select broswer name,count(1) value, DATE_FORMAT(operatetime, '%Y-%m-%d') date from t_s_log  where id = <%="'${abc}'"%>  group by broswer。（如果id字段为字符串类型）<br/>
							&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">注：必要用 name,value,date 列。 date 为X轴数据,value 为Y轴数据,name为分组数据名称</font><p/>
			</td>
		</tr>
		<tr>
