	...
	<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
		<tr>
			<td align="right" width="25%" nowrap>
                <label class="Validform_label">  <t:mutiLang langKey="管理员"/>:  </label>
            </td>
			<td class="value" width="85%">
                <c:if test="${user.id!=null }"> ${user.userName } </c:if>
                <c:if test="${user.id==null }">
                    <input id="userName" class="inputxt" name="userName" validType="t_s_base_user,userName,id" value="${user.userName }" datatype="s1-64" />
                    <span class="Validform_checktip"> <t:mutiLang langKey="管理员范围在1~64位字符"/></span>
                </c:if>
            </td>
		</tr>
		<tr>
			<td align="right" width="10%" nowrap><label class="Validform_label"> <t:mutiLang langKey="common.surname"/>: </label></td>
		...
		<c:if test="${user.id==null }">
			<tr>
				...
				<%-- <td class="value">
                    <input type="password" class="inputxt" value="" name="password"  datatype="*6-8" plugin="passwordStrength"/>
                ...
                <td class="value">
                    <input type="password" class="inputxt" value="" name="password"  datatype="adminPassword" with="userName" plugin="passwordStrength"/>
          			<span class="passwordStrength" style="display: none;">
                        <span><t:mutiLang langKey="common.weak"/></span>
                        <span><t:mutiLang langKey="common.middle"/></span>
                        <span class="last"><t:mutiLang langKey="common.strong"/></span>
		...
		<tr>
			<td align="right"><label class="Validform_label"> <t:mutiLang langKey="department.manage"/>: </label></td>
			<td class="value">
				 <%--update-start--Author:jg_renjie  Date:20160320 for：#942 【组件封装】组织机构弹出模式，目前是列表，得改造成树方式--%>
                <%--update-start--Author:zhangguoming  Date:20140826 for：将combobox修改为combotree--%>
                ...
                <a href="#" class="easyui-linkbutton" plain="true" icon="icon-redo" id="departRedo" onclick="callbackClean()">清空</a>
                 <%--update-end--Author:jg_renjie  Date:20160320 for：#942 【组件封装】组织机构弹出模式，目前是列表，得改造成树方式--%>
                <span class="Validform_checktip"><t:mutiLang langKey="机构可多选"/></span>
            </td>
		</tr>
		<tr>
