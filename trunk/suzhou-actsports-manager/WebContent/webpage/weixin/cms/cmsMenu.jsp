...
<html>
 <head>
  ...
  <link type="text/css" rel="stylesheet" href="plug-in/easyui/themes/icon.css" />
  ...
  $(function () {
	  ...
	  $('#parentId').combotree({
			url : 'cmsMenuController.do?treeMenu&lang=${lang}'
  ...
  /** function changeVisitType(obj){
  if (obj == '2'){
	  $('#trlinkUrl').show();
	  $("#linkUrl").attr("datatype","*");
  }else {
	  $("#linkUrl").removeAttr("datatype");
	  $('#trlinkUrl').hide();
  }
}**/
 		...
 		<div class="media_edit_area" id="js_appmsg_editor">	
			<div class="appmsg_editor" style="margin-top: 0px;">
		 		<div class="inner">
					...
					<input type="hidden" name="orgCode" value="${columnPage.orgCode}">
					...
					<table cellpadding="0" cellspacing="1" class="formtable">
						<tr>
							...
							<td class="value">
								<select id="parentId" name="parentId" value="${parentId}" />
							</td>
						</tr>
						<tr>
							<td align="right">
						...
						<tr>
							...
							<td class="value">
								<input class="inputxt" id="menuCode" name="menuCode" style="width: 300px" value="${columnPage.menuCode}" <c:if test="${columnPage.menuCode != null && columnPage.menuCode != '' }">readonly="readonly"</c:if> />
								<span class="Validform_checktip"></span>
								<label class="Validform_label" style="display: none;">栏目编码</label>
							</td>
						</tr>
						<tr>
							<td align="right">
							...
							<td class="value">
								<%-- <t:dictSelect dictField="type" field="type" extendJson="{onchange:'changeMenuType(this.value)'}" typeGroupCode="cms_menu" hasLabel="false" defaultVal="${columnPage.type}"></t:dictSelect> --%>
								<t:dictSelect dictField="type" field="type" extendJson="{onchange:'changeMenuType(this.value)'}" typeGroupCode="cms_menu" hasLabel="false" defaultVal="${columnPage.type}"></t:dictSelect>
								<span class="Validform_checktip"></span>
						...
						<tr id="selArticle">
							...
							<td class="value">
								<input id="articleId" name="articleId" type="hidden" value="${articleId}" style="width: 150px" class="inputxt" >
								<input id="articleTitle" name="articleTitle" value="${articleTitle}" type="text" style="width: 150px" class="inputxt" readonly="true">
								<t:choose hiddenName="articleId" hiddenid="id" url="cmsArticleController.do?selArticle" name="cmsArticleList" 
								 icon="icon-search" title="文章列表" inputTextname="articleTitle" textname="title" isclear="true"></t:choose>
							</td>
						</tr>
						
						<c:if test="${columnPage.type!='02'}">
						<script type="text/javascript">
						...
						<tr>
							...
							<td class="value">
								<t:dictSelect field="showFlag" typeGroupCode="yesorno" hasLabel="false" defaultVal="${columnPage.showFlag!=null?columnPage.showFlag:'Y'}" ></t:dictSelect>
								<span class="Validform_checktip"></span>
						...
						<!--<tr>
							...
							<td class="value">
								<t:dictSelect extendJson="{onchange:'changeVisitType(this.value)'}" field="visitType" typeGroupCode="visitType" 
									hasLabel="false" defaultVal="${visitType!=null?visitType:1}" ></t:dictSelect>
								<span class="Validform_checktip"></span>
								<label class="Validform_label" style="display: none;">访问类型</label>
							</td>
						</tr>  -->
						<tr id="trlinkUrl" style="display:none">
							...
							<td class="value">
								<input class="inputxt" id="linkUrl" name="linkUrl" nullmsg="录入链接URL"  style="width: 300px" value="${columnPage.linkUrl}" >
								<span class="Validform_checktip"></span>
