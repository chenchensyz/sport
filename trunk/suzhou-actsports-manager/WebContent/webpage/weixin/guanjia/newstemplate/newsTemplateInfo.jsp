  ...
  <t:formvalid formid="newstemplate" dialog="true" usePlugin="password" layout="table" action="newsTemplateController.do?doSave">
   ...
   <table style="width:600px;" cellpadding="0" cellspacing="1" class="formtable">
    ...
    <tr>
     ...
     <td class="value">
          <select name="weixingroup" id="weixingroup">
          	<option value="">---请选择---</option>
          	<c:forEach items="${grouplist}"  var="glist">
          		<option value="${glist.id}" <c:if test="${glist.id == gId }">selected="selected"</c:if> >${glist.name} </option>
          	</c:forEach>
          </select>
      	  <span class="Validform_checktip">请输入选择的分组！</span>
     </td>
    </tr>
