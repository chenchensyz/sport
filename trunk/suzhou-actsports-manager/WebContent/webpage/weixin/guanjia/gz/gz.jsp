...
<body style="overflow-y: hidden" scroll="no">
	<t:formvalid formid="gzfrom" dialog="true" usePlugin="password"
		layout="table" action="subscribeController.do?su">
		<input id="id" name="id" type="hidden" value="${id}">
		<input id="accountid" name="accountid" type="hidden"
			value="${subscribe.accountid}">
		<input id="templateName" name="templateName" type="hidden"
			value="${subscribe.templateName}" />
		<table style="width: 600px;" cellpadding="0" cellspacing="1"
			...
			<tr>
				...
				<td class="value"><select name="msgType" id="msgType">
						<option value="text"
							<c:if test="${subscribe.msgType=='text'}">selected="selected"</c:if>>文本消息</option>
						<option value="news"
							<c:if test="${subscribe.msgType=='news'}">selected="selected"</c:if>>图文消息</option>
				</td>
			</tr>

			<tr>
				<td align="right"><label class="Validform_label"> 消息模板:</label></td>
				<td class="value" id="textId"><select name="templateId"
					id="textContent">
						<c:forEach items="${textList}" var="textVal">
							<option value="${textVal.id}" name="textMsg"
								<c:if test="${templateId==textVal.id}">selected="selected"</c:if>>${textVal.templateName}</option>
						</c:forEach>
				</select></td>
				<td class="value" style="display: none" id="newsId"><select
					name="templateId" id="newsContent" disabled="disabled">
						<c:forEach items="${newsList}" var="newsVal">
							<option value="${newsVal.id}"
								<c:if test="${templateId==newsVal.id}">selected="selected"</c:if>>${newsVal.templateName}</option>
						</c:forEach>
				</select></td>
			</tr>
