...
public class PrizeController {
	...
	public void exportExcel(String id, HttpServletResponse response) throws Exception{
		...
		List<PrizeResultVo> ervList = prizeService.getPrizeResultVoByPrizeId(id);
		
		List<String> cellNameList = new ArrayList<>();
		cellNameList.add("用户编号");
		cellNameList.add("抽奖活动编号");
		cellNameList.add("抽奖名称");
		cellNameList.add("中奖日期");
		cellNameList.add("抽奖项目");
		...
		List<String> params;
		for(PrizeResultVo erv:ervList){
			params = new ArrayList<>();
			params.add(erv.getUserId());
			params.add(erv.getPrizeId());
			params.add(erv.getPrizeTitle());
        ...
        response.setHeader("Content-disposition", "attachment;filename=" + java.net.URLEncoder.encode(ervList.get(0).getPrizeTitle(), "UTF-8") +".xls");    
	...
	public ModelAndView toexchange(String sn,  HttpServletRequest request, HttpServletResponse response)  throws Exception{
		 ...
		 String sessionAuth = (String) request.getSession().getAttribute("auth");  
	        ...
	        if (sessionAuth != null) {  
	            ...
	            String sql = "select sn,type,status from t_act_prize_sn where sn=?";
        		Map<String, Object>  prize = systemService.findOneForJdbc(sql,sn);
        		request.setAttribute("prize", prize);
        		return new ModelAndView("prize/toexchange");
	        } else {  
	...
	public String doexchange(String sn, HttpServletRequest request) throws Exception{
		String sql = "update t_act_prize_sn set status=2 ,update_date=now() where sn=?";
