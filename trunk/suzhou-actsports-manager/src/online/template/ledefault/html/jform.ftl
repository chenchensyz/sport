...
<head>
  ...
  <link rel="stylesheet" href="online/template/ledefault/css/vendor.css"></link>
  <link rel="stylesheet" href="online/template/ledefault/css/bootstrap-theme.css"></link>
			    ...
			    <div class="con-wrapper" id="con-wrapper1" style="display: block;">
			      <div class="row form-wrapper">
			        <#list columns as po>
			          ...
			          <div class="col-xs-3 meta-col-xs-3">
						...
						<#elseif po.show_type=='text'>
							<input id="${po.field_name}" ${po.extend_json?if_exists} name="${po.field_name}" type="text"
				               ...
				               <#if po.field_valid_type?if_exists?html != '' || po.is_null != 'Y'>
				               		<i style="position: absolute;right: 5px;top: 3px;color: red;font-size: 14px;">*</i>
				               </#if>
						...
						<#elseif po.show_type=='list'>
							<@DictData name="${po.dict_field?if_exists?html}" text="${po.dict_text?if_exists?html}" tablename="${po.dict_table?if_exists?html}" var="dataList">
								<select id="${po.field_name}" ${po.extend_json?if_exists} class="form-control" name="${po.field_name}" <#if po.operationCodesReadOnly?if_exists>onfocus="this.defOpt=this.selectedIndex" onchange="this.selectedIndex=this.defOpt;"</#if><#if po.is_null != 'Y'>datatype="*"</#if> >
									...
									<#if data['${tableName}']['${po.field_name}']??>
										<#if dictdata.typecode?if_exists?html=="${data['${tableName}']['${po.field_name}']?if_exists?html}"> selected="selected" </#if>>
									<#else>
										<#if dictdata.typecode?if_exists?html=="${po.field_default?if_exists?html}"> selected="selected" </#if>>
									</#if>
										${dictdata.typename?if_exists?html}
									</option> 
									</#list> 
						...
						<#elseif po.show_type=='popup'>
               				 <input name="${po.field_name}" type="hidden" value="${data['${tableName}']['${po.field_name}']?if_exists?html}" id="${po.field_name}">
               				 <#if data['${tableName}']['${po.field_name}']?if_exists?html ==''>
               				 	<input name="dict_${po.dict_field}" type="text" class="form-control" value="" id="dict_${po.dict_field}" readonly="readonly"  <#if po.field_valid_type?if_exists?html != ''>
					               datatype="${po.field_valid_type?if_exists?html}"
					               <#else>
					               <#if po.is_null != 'Y'>datatype="*"</#if> 
					               </#if>/> 
               				 <#else>
               				 	<@DictData name="${po.dict_field?if_exists?html}" text="${po.dict_text?if_exists?html}" where="${data['${tableName}']['${po.field_name}']?if_exists?html}" tablename="${po.dict_table?if_exists?html}" var="dataList">
									<#list dataList as dictdata> 
										<#if dictdata.typecode?if_exists?html=="${data['${tableName}']['${po.field_name}']?if_exists?html}">
											<input name="dict_${po.dict_field}" type="text" class="form-control" value="${dictdata.typename?if_exists?html}" id="dict_${po.dict_field}" readonly="readonly"  
											<#if po.field_valid_type?if_exists?html != ''>
					               				datatype="${po.field_valid_type?if_exists?html}"
					               			<#else>
					               			<#if po.is_null != 'Y'>datatype="*"</#if> 
                			 ...
                			 <a href="#" class="easyui-linkbutton l-btn l-btn-plain" plain="true" icon="icon-search" onclick="choose_${po.dict_table}()" id="pick">
                			 	选择
                			 </a>
                			 <a href="#" class="easyui-linkbutton l-btn l-btn-plain" plain="true" icon="icon-redo" onclick="clearAll_${po.dict_table}();" id="pickclear">
                			 	清空
                			 </a>
                			 <script type="text/javascript">
                			 	var windowapi = frameElement.api, W = windowapi.opener;
                			 	function choose_${po.dict_table}(){
                			 		var url = 'cgReportController.do?list&id=${po.dict_table}';
                			 		var initValue = $('#${po.field_name}').val();
                			 		url += '&ids='+initValue;
                			 		if(typeof(windowapi) == 'undefined'){
                			 			$.dialog({
                			 				...
                			 				opacity : 0.4,
                			 				button : [
                			 					{name : '确定',callback : clickcallback_${po.dict_table},focus : true}, 
                			 	...
                			 	function clearAll_${po.dict_table}(){
                			 		if($('#dict_${po.dict_field}').length>=1){
                			 			$('#dict_${po.dict_field}').val('');
                			 			$('#dict_${po.dict_field}').blur();
                			 		}
                			 		if($("input[name='dict_${po.dict_field}']").length>=1){
                			 			$("input[name='dict_${po.dict_field}']").val('');
                			 			$("input[name='dict_${po.dict_field}']").blur();
                			 		}
                			 		$('#${po.field_name}').val("");
                			 	}
                			 	function clickcallback_${po.dict_table}(){
                			 		iframe = this.iframe.contentWindow;
                			 		var dict_${po.dict_text}=iframe.get${po.dict_table}ListSelections('${po.dict_text}');
                			 		var poptextfiled = $('#${po.field_name}').parent().find("input[name='dict_${po.dict_field}']");
                			 		if(poptextfiled.length>=1){
                			 			poptextfiled.val(dict_${po.dict_text});
                			 			poptextfiled.blur();
                			 		}
                			 		//if($('#dict_${po.dict_field}').length>=1){
                			 		//	$('#dict_${po.dict_field}').val(dict_${po.dict_text});
                			 		//	$('#dict_${po.dict_field}').blur();
                			 		//}
                			 		//if($("input[name='dict_${po.dict_field}']").length>=1){
                			 		//	$("input[name='dict_${po.dict_field}']").val(dict_${po.dict_text});
                			 		//	$("input[name='dict_${po.dict_field}']").blur();
                			 		//}
                			 		var dict_${po.dict_field} =iframe.get${po.dict_table}ListSelections('${po.dict_field}');
                			 		if (dict_${po.dict_field}!== undefined &&dict_${po.dict_field}!=""){
                			 			$('#${po.field_name}').val(dict_${po.dict_field});
                			 		}
                			 	}
                			 </script>
						
						
						<#elseif po.show_type=='ppCheckox'>
               				 <input name="${po.field_name}" type="hidden" value="${data['${tableName}']['${po.field_name}']?if_exists?html}" id="${po.field_name}">
               				 <#if data['${tableName}']['${po.field_name}']?if_exists?html ==''>
               				 	<input name="dict_${po.dict_field}" type="text" class="form-control" value="" id="dict_${po.dict_field}" readonly="readonly"  <#if po.field_valid_type?if_exists?html != ''>
					               datatype="${po.field_valid_type?if_exists?html}"
               				 ...
               				 <#else>
               				 	<@DictData name="${po.dict_field?if_exists?html}" text="${po.dict_text?if_exists?html}" type="ppCheckox" where="${data['${tableName}']['${po.field_name}']?if_exists?html}" tablename="${po.dict_table?if_exists?html}" var="dataList">
									<#list dataList as dictdata> 
										<#if dictdata.typecode?if_exists?html=="${data['${tableName}']['${po.field_name}']?if_exists?html}">
											<input name="dict_${po.dict_field}" type="text" class="form-control" value="${dictdata.typename?if_exists?html}" id="dict_${po.dict_field}" readonly="readonly"  
											<#if po.field_valid_type?if_exists?html != ''>
                			 ...
                			 <script type="text/javascript">
                			 	var windowapi = frameElement.api, W = windowapi.opener;
                			 	function choose_${po.dict_table}(){
                			 		var url = 'cgReportController.do?list&id=${po.dict_table}&checkbox=Y';
                			 		var initValue = $('#${po.field_name}').val();
                			 		url += '&ids='+initValue;
                			 		if(typeof(windowapi) == 'undefined'){
                			 			$.dialog({
						...
						<#elseif po.show_type=='file'>
								<table>
									<#list imageList as imageB>
										...
										<td>${imageB['title']}</td>
										<td><a href="commonController.do?viewFile&fileid=${imageB['fileKey']}&subclassname=com.hbasesoft.framework.manager.web.cgform.entity.upload.CgUploadEntity" title="下载">下载</a></td>
										<td><a href="javascript:void(0);" onclick="openwindow('预览','commonController.do?openViewFile&fileid=${imageB['fileKey']}&subclassname=com.hbasesoft.framework.manager.web.cgform.entity.upload.CgUploadEntity','fList',700,500)">预览</a></td>
						...
						<#elseif po.show_type=='image'>
								<table>
									<#if data['${tableName}']['${po.field_name}']??>
										<tr style="height:34px;">
										<td><img src="${data['${tableName}']['${po.field_name}']}" height="100px"></td>
										</tr>
									<#else>
										<#list filesList as fileB>
											<#if fileB['field'] == po.field_name>
											<tr style="height:34px;">
											<td><img src="${fileB['fileKey']}" height="100px"></td>
											<td><a href="commonController.do?viewFile&fileid=${fileB['fileKey']}&subclassname=com.hbasesoft.framework.manager.web.cgform.entity.upload.CgUploadEntity" title="下载">下载</a></td>
											<td><a href="javascript:void(0);" onclick="openwindow('预览','commonController.do?openViewFile&fileid=${fileB['fileKey']}&subclassname=com.hbasesoft.framework.manager.web.cgform.entity.upload.CgUploadEntity','fList',700,500)">预览</a></td>
			        ...
			        <#if (columns?size>10)>
						<#if (po_index%2==0)&&(!po_has_next)>
							<div class="col-xs-3 text-center"><b></b></div>
			         		<div class="col-xs-3 meta-col-xs-3"></div>
						</#if>
						<#if (po_index%2!=0)||(!po_has_next)>
							</div>
						</#if>
